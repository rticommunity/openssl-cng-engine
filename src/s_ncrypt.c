...
#define CMN_THIS_FILE "src/s_ncrypt.c"
...
const char *ncrypt_store_id = "engine-ncrypt";
const char *ncrypt_store_name = "CryptoAPI: Next Gen (CNG) NCrypt STORE Engine";
...
ncrypt_store_bind(ENGINE *engine)
{
    ...
    if (ENGINE_set_id(engine, ncrypt_store_id) != 1) {
        S_NCRYPT_osslerr(ENGINE_set_id, "NCrypt engine binding");
        goto done;
    }

    if (ENGINE_set_name(engine, ncrypt_store_name) != 1) {
